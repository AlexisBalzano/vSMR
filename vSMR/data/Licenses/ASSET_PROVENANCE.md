...
| AVISO airport geometry | `AVISO/<ICAO>.geojson` | 160 airport maps imported on 2026-09-17 from the user-supplied vSMR AVISO Converter GeoJSON folder (local GNG/KMZ sources). On 2026-09-18 LFPG's historical East/West arrows were restored from commit `0066b62`; its original airport features and the other 159 maps remain unchanged. Reviewed hashes and the original LFPG import hash are recorded in `vSMR/tests/fixtures/aviso-set-20260917.json` (earlier import records remain in Git history). Upstream GPL-3.0 license retained as `France-Ground-Layouts.txt`; local and historical source redistribution terms still require review | Verification required |
