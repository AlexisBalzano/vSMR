...
| AVISO airport geometry | `AVISO/<ICAO>.geojson` | 192 airport maps imported on 2026-09-15 from the user-supplied vSMR AVISO Converter GeoJSON folder (local GNG/KMZ sources). Source hashes and the single invalid LFLG line removed during validation are recorded in `docs/aviso-set-20260915.json`. Upstream GPL-3.0 license retained as `France-Ground-Layouts.txt`; local and historical source redistribution terms still require review | Verification required |
