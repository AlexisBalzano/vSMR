...
// EuroScope synchronizes the controller assigned speed and no aircraft needs one
// while it is on the ground, so vSMR encodes the ground states EuroScope has no
// status for in a reserved band. Every vSMR client reads the same value and shows
